...
struct calc_accounts {
  int id;
  char calc[4096];
};

struct calc_accounts accounts[1024];
        ...
        if (poll_fds[i].fd == listenfd) {
          ...
          if (fgets(buf, sizeof(buf), stdin)) {
            if (strncmp(buf, "EXIT", 4) == 0) {
              ...
              printf("SINGURA COMANDA PERMISA ESTE: EXIT\n");
            }
          }
        } else {
          ...
          if (rc == 0) {
            ...
            // Cele 3 comenzi posibile: CALC, LIST si EXIT

            // CALC X OPERATOR Y
            struct chat_packet response;
            memset(&response, 0, sizeof(response));

            if (strncmp(received_packet.message, "CALC", 4) == 0) {
              int x, y;
              char op;
              sscanf(received_packet.message, "CALC %d %c %d", &x, &op, &y);
              ...
              if (op == '+') {
                ...
                if (y == 0) {
                  strcpy(response.message, "Nu este permisa impartirea la 0.\n");
                  response.len = strlen(response.message) + 1;
                  
                  send_all(poll_fds[i].fd, &response, sizeof(response));
                  continue;
                }
              ...
              // Acum am toata operatia calculata, o pun in .calc
              char curr_calc[1024];
              sprintf(curr_calc, "%d %c %d = %d\n", x, op, y, res);
              strcat(accounts[poll_fds[i].fd].calc, curr_calc);

              // Trimit pachetul ce rezultatul
              sprintf(response.message, "%d %c %d = %d\n", x, op, y, res);
              response.len = strlen(response.message) + 1;

              send_all(poll_fds[i].fd, &response, sizeof(response));
            } else if (strncmp(received_packet.message, "LIST", 4) == 0) {
              // Trimit tot ce e in accounts.calc
              strcpy(response.message, accounts[poll_fds[i].fd].calc);
              response.len = strlen(response.message) + 1;
